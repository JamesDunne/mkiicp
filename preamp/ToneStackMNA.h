 ...
 * equations at runtime for maximum accuracy and robustness.
 *
...
public:
    ...
    void setParams(double treble, double mid, double bass, double volume) {
        ...
        m_treble = std::clamp(treble, 0.0, 1.0);
        m_bass = std::clamp(bass, 0.0, 1.0);
        m_mid = std::clamp(mid, 0.0, 1.0);
        m_volume = std::clamp(volume, 0.0, 1.0);
    }

    void prepare(double sampleRate, double R_load) {
        ...
        double treble_t = m_treble * travel_max + travel_min;
        double volume_t = m_volume * travel_max + travel_min;
    ...
    /**
     * @brief Processes one audio sample.
...
private:
    // Member variables are prefixed with 'm_' for clarity.
    double m_treble;
    double m_bass;
    double m_mid;
    double m_volume;
    ...
    void invertMatrix(double a[7][7], double inv[7][7]) {
        double temp[7][14];
        for (int i = 0; i < 7; i++) {
            for (int j = 0; j < 7; j++) {
                temp[i][j] = a[i][j];
