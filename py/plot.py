...
# --- 3. Constrained Polynomial Fitting (with Smart Initial Guess) ---
...
def objective_func(p, x, y):
    """The function to minimize: the sum of squared errors."""
    return np.sum((poly_func(p, x) - y)**2)

print("--- Starting Robust Constrained Optimization ---")
print("Strategy: Use np.polyfit for a smart initial guess, then run constrained minimization.")
...
# STAGE 1: Get a smart initial guess
p0_1 = np.polyfit(vg_seg1, vp_seg1, poly1_deg)
# STAGE 2: Run constrained optimization
constraints1 = [
...
if not res1.success: print(f"WARNING: Segment 1 optimization failed: {res1.message}")
...
# STAGE 1: Get a smart initial guess
p0_2 = np.polyfit(vg_seg2, vp_seg2, poly2_deg)
# STAGE 2: Run constrained optimization
constraints2 = [
...
if not res2.success: print(f"WARNING: Segment 2 optimization failed: {res2.message}")
...
# STAGE 1: Get a smart initial guess
p0_3 = np.polyfit(vg_seg3, vp_seg3, poly3_deg)
# STAGE 2: Run constrained optimization
constraints3 = [
...
if not res3.success: print(f"WARNING: Segment 3 optimization failed: {res3.message}")
...
ax1.plot(vg_model_fine, vp_model_fine, '--', color='red', linewidth=2.5, label='Final Polynomial Model')
...
ax1.set_title('12AX7 Triode Model: Final Robust and Accurate Polynomial Fit', fontsize=16)
